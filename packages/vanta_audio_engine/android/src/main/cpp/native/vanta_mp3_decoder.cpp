...
bool VantaMp3Decoder::OpenLocalPath(const char *path) {
...
bool VantaMp3Decoder::Seek(uint64_t position_ms) {
...
uint64_t VantaMp3Decoder::PositionMs() const {
  if (!ready_ || sample_rate_ == 0) {
    return 0;
  }
  ma_uint64 cursor = 0;
...
int64_t VantaMp3Decoder::DurationMs() const {
...
ma_uint64 VantaMp3Decoder::ReadPcmFrames(void *output, ma_uint32 frame_count) {
  if (!ready_) {
    return 0;
  }
...
bool VantaMp3Decoder::IsReady() const { return ready_; }

ma_format VantaMp3Decoder::OutputFormat() const { return decoder_.outputFormat; }

ma_uint32 VantaMp3Decoder::OutputChannels() const { return channels_; }

ma_uint32 VantaMp3Decoder::SampleRate() const { return sample_rate_; }

ma_uint64 VantaMp3Decoder::TotalFrames() const { return total_frames_; }
} // namespace vanta_audio_engine
