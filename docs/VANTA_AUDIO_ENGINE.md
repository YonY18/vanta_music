...
2. Use **Vanta Native Engine (Experimental)** only to exercise the native WAV/FLAC playback foundation, including local Android `content://` sources validated by the platform layer, native position/duration events, and seek/completion behavior.
...
- Keeps the native output device open across compatible native-to-native local WAV/FLAC loads when sample rate, channel count, and PCM format match. This is an experimental output lifecycle optimization only; it is not full gapless playback, preloading, or crossfade.
...
| Duration | WAV/FLAC duration is calculated from total PCM frames and sample rate when available. The bridge emits duration after load; unknown duration remains `null`. |
...
| Buffer policy | Native FLAC/WAV uses an internal stability-oriented PCM ring buffer policy. The helper is clamp-tested between 250 ms and 1000 ms, but production currently uses the stable-music default: 750 ms capacity with up to 500 ms initial fill. No external/runtime adjustment is exposed yet. |
...
6. Play MP3, OGG/Opus, M4A/AAC, remote, and Navidrome/Subsonic tracks to confirm they still use the current engine fallback.
...
- [ ] MP3/remote/Navidrome/Subsonic: remain on the current engine fallback path; native does not claim these sources.
...
| `.mp3`, `.ogg`, `.opus`, `.m4a`, `.aac`, other | `unsupported` | Current-engine fallback. Future MP3/OGG/Opus work should be localized here plus dedicated decoder wrappers. |
